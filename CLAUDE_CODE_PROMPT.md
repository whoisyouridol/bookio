...
You are analyzing a **multi-tenant beauty salon booking platform (BookVisit/BeautySalonBooking)** — a hybrid SaaS with subdomain routing per salon. Your job is to:
...
BookVisit is a hybrid SaaS for beauty salon bookings. It supports 4 user types with completely different UIs and functionality:

**1. Unauthorized / Guest User:**
- Sees the marketplace (all salons) if no subdomain, or a single salon page if on `{slug}.bookvisit.ge`
...
- Subdomain routing: `{slug}.bookvisit.ge` per salon with dynamic theming
